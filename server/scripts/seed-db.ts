...
import { sequelize, User, Category, Product } from '../src/models';
...
async function seedDatabase(): Promise<void> {
  try {
    ...
    const hashedPassword: string = await bcrypt.hash('admin123', salt);
    ...
    // Create regular user
    const userPassword: string = await bcrypt.hash('user123', salt);
